...
func setupTestDBManager(t *testing.T) interfaces.RepositoryManager {
...
func TestBuildSQLQuery(t *testing.T) {
	...
	query := repo.BuildSQLQuery(tableName, params, args)
...
func TestAddRows(t *testing.T) {
	...
	err := repo.AddRows("users", args)
	if err != nil {
		t.Fatalf("Ошибка при добавлении строки в таблицу users: %v", err)
	}

	// Удаление тестового пользователя после добавления
	defer repo.DeleteRows("users", map[string][]string{"email": {"test@example.com"}})

	// Проверяем, что данные добавлены
	rows, err := repo.GetRows("users", []string{"email", "password"}, map[string][]string{"email": {"test@example.com"}})
	if err != nil {
...
func TestGetRows(t *testing.T) {
	...
	err := repo.AddRows("users", map[string][]string{"email": {"test@example.com"}, "password": {"password"}})
	if err != nil {
...
func TestDeleteRows(t *testing.T) {
	...
	// Добавляем тестового пользователя
	err := repo.AddRows("users", map[string][]string{"email": {"delete_test@example.com"}, "password": {"password"}})
	if err != nil {
		t.Fatalf("Ошибка при добавлении тестового пользователя: %v", err)
	}

	// Удаляем тестового пользователя
	err = repo.DeleteRows("users", map[string][]string{"email": {"delete_test@example.com"}})
	if err != nil {
		t.Fatalf("Ошибка при удалении строки из таблицы users: %v", err)
	}

	// Проверяем, что данных больше нет
	rows, err := repo.GetRows("users", []string{"email"}, map[string][]string{"email": {"delete_test@example.com"}})
	if err != nil {
