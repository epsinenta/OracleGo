...
func TestGetPlayerOnHeroWinrate(t *testing.T) {
	...
	// Тестовые данные
	players := []entities.Player{{Value: "Ame"}}
	heroes := []entities.Hero{{Value: "Anti-Mage"}}

	// Выполнение функции
	result, err := svc.GetPlayerOnHeroWinrate(players, heroes)
	if err != nil {
...
func TestGetPlayerCountOnHero(t *testing.T) {
	...
	// Выполнение функции
	result, err := svc.GetPlayerCountOnHero(players, heroes)
	if err != nil {
